...
import { ShieldCheck, Activity, Coins, Wallet, TrendingUp } from "lucide-react";
...
const accentMap: Record<Role, { grad: string; chip: string; dot: string }> = {
  manufacturer: {
  ...
  carrier: {
    grad: "bg-gradient-to-br from-accent to-primary",
...
export const RoleHeader = ({ role, label, subtitle }: RoleHeaderProps) => {
  const cfg = accentMap[role];
  ...
  const role2 = getRoleFromPath(location.pathname);
  ...
  const { data: tokenBalance, isError: tokenError } = useReadContract({
    address: role2?.tokenAddress,
    abi: SupplyChainTokenAbi,
    functionName: "balanceOf",
    args: address ? [address] : undefined,
    query: {
      enabled: !!address && !!role2?.tokenAddress,
  ...
  const isWholesaler = role2?.label === "WHL";
    ...
    <header className="glass-strong sticky top-0 z-30 flex h-16 items-center justify-between rounded-2xl px-5">
      <div className="flex items-center gap-4">
        <Link to="/" className="flex items-center gap-2.5">
          <div
            className={`relative flex h-9 w-9 items-center justify-center rounded-xl ${cfg.grad} shadow-glow`}
          >
            <ShieldCheck
        ...
        <motion.div
          ...
          className={`flex items-center gap-2 rounded-full border px-3 py-1.5 text-xs font-semibold ${cfg.chip}`}
        >
      ...
      <div className="flex items-center gap-3">
        ...
        {isConnected && address && role && (
          <motion.div
            ...
            className="flex items-center gap-3 rounded-xl border border-border/60 bg-surface-2/80 px-4 py-auto"
          >
            {/* Role Badge */}
            <div
              className={`flex h-7 items-center justify-center rounded-md bg-surface-3 px-2.5 text-[10px] font-bold tracking-wider ${role2.color}`}
            >
              {role2.label}
            </div>
